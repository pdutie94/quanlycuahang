  ...
  <section class="space-y-4">
    ...
    <template v-else-if="purchases.detail">
      ...
      <div class="rounded-2xl border border-black/10 bg-white p-4">
        ...
        <div class="space-y-2">
          <article v-for="item in purchases.detail.items" :key="item.id" class="rounded-xl border border-black/10 p-3">
            <div class="flex items-start justify-between gap-2">
              <h4 class="font-medium">{{ item.product_name }}</h4>
              <p class="text-xs text-ink/60">{{ item.unit_name }}</p>
            </div>
            <div class="mt-2 grid grid-cols-3 gap-2 text-sm">
              <p class="rounded-lg bg-black/5 px-2 py-1">SL: <strong>{{ item.qty }}</strong></p>
              <p class="rounded-lg bg-black/5 px-2 py-1">Giá nhập: <strong>{{ formatMoney(item.price_cost) }}</strong></p>
              <p class="rounded-lg bg-black/5 px-2 py-1">Tiền: <strong>{{ formatMoney(item.amount) }}</strong></p>
            </div>
          </article>
        </div>
      </div>
