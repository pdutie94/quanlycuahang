      ...
      <table class="min-w-full text-sm">
        <thead class="bg-black/5 text-left text-xs uppercase tracking-wider text-ink/60">
          <tr>
            ...
            <th class="px-3 py-2">Giá vốn hiện tại</th>
            <th class="px-3 py-2">Giá vốn đơn vị</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr v-else-if="rows.length === 0" class="border-t border-black/5">
            <td class="px-3 py-4 text-center text-ink/60" colspan="5">Không có dòng thiếu giá vốn.</td>
          </tr>
          <tr v-else v-for="row in rows" :key="row.id" class="border-t border-black/5">
            <td class="px-3 py-2">{{ row.order_code }}</td>
            <td class="px-3 py-2">{{ row.product_name }} ({{ row.unit_name }})</td>
            <td class="px-3 py-2">{{ row.qty }}</td>
            <td class="px-3 py-2 text-red-600">{{ formatMoney(row.price_cost) }}</td>
            <td class="px-3 py-2">{{ formatMoney(row.current_unit_price_cost) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
