      ...
      <div v-if="open" class="app-modal-overlay app-modal-open" @click.self="$emit('close')">
        <div class="app-modal-sheet app-modal-sheet-fill">
          ...
          <div class="app-modal-body app-modal-body-fill pt-2 pb-3">
            <div class="mb-2">
              <input v-model="keyword" type="search" :placeholder="searchPlaceholder" class="app-input" />
            </div>
              ...
              >
                <div class="min-w-0 flex-1">
                  <div class="font-medium text-slate-900">
                    {{ item.name }}<span v-if="item.unitName" class="text-slate-500"> - {{ item.unitName }}</span>
                  </div>
                  <div v-if="item.subLabel" class="mt-0.5 text-sm text-slate-500">{{ item.subLabel }}</div>
                  <div class="mt-0.5 text-sm font-medium text-brand-700">{{ formatMoney(item.price) }}</div>
                </div>
                <span
                  class="inline-flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-md border text-xs font-semibold"
                  :class="isSelected(item.id) ? 'border-brand-600 bg-brand-600 text-white' : 'border-slate-300 bg-white text-transparent'"
                >✓</span>
              </button>
              <div v-if="!filtered.length" class="p-3 text-center text-sm text-slate-500">
          ...
          <div class="app-modal-footer">
            ...
            <button type="button" class="app-btn-primary" :disabled="!selectedIds.length" @click="$emit('apply')">
              {{ applyLabel }}{{ selectedIds.length ? ` (${selectedIds.length})` : '' }}
            </button>
          </div>
        </div>
      </div>
