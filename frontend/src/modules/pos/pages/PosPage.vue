...
const parsePriceShorthand = (raw: any) => {
...
const roundDownThousand = (value: any) => {
...
const manualRoundTotal = () => {
    // Không reset discount/surcharge, chỉ làm tròn paymentAmount
    paymentAmount.value = formatMoneyInput(
        roundDownThousand(
            subtotal.value - discountAmount.value + surchargeValue.value,
        ),
    );
};
...
const selectorItems = computed<SelectorItem[]>(() =>
    products.value.map((product) => {
        const defaultUnit = getDefaultUnit(product.id);
        return {
...
const baseUnitsForManual = computed(() => {
    ...
    for (const units of Object.values(productUnitsByProduct.value)) {
        for (const unit of units as any[]) {
            const name = String(unit.unit_name || "").trim();
            if (name && !seen.has(name)) {
                seen.add(name);
                result.push({ id: unit.unit_id || name, name });
            }
        }
    }
    return result;
...
const selectedCustomer = computed(() => {
    ...
    return (
        customers.value.find(
            (customer) =>
                Number(customer.id) === Number(selectedCustomerId.value),
        ) || null
    );
});

const showExistingCustomerPicker = computed(
...
watch(customerMode, (mode) => {
    ...
    if (mode === "existing") {
        pendingCustomerId.value = selectedCustomerId.value
...
const getUnits = (productId: any) =>
    productUnitsByProduct.value[String(productId)] ||
    productUnitsByProduct.value[productId] ||
    [];
const getCartItemUnit = (item: any) =>
    getUnits(item.product_id).find(
        (unit: any) => Number(unit.unit_id) === Number(item.unit_id),
    ) || null;
const getCartItemUnitName = (item: any) =>
    getUnits(item.product_id).find(
        (unit: any) => Number(unit.unit_id) === Number(item.unit_id),
    )?.unit_name || "";
const getCartItemBasePrice = (item: any) =>
    Number(item?.base_price || item?.price || 0);
const getCartItemStep = (item: any) => {
...
const getCartItemMinQty = (item: any) => getCartItemStep(item);
const formatQtyValue = (value: any) =>
    Number(value || 0)
        .toFixed(4)
        .replace(/\.?0+$/, "");
const normalizeCartQty = (item: any) => {
    ...
    if (step === 1) {
        item.quantity = formatQtyValue(
            Math.max(minQty, Math.round(currentQty)),
        );
        return;
    }
...
const getManualQtyStep = (item: any) => 1 / 10 ** getManualQtyPrecision(item);
const getManualQtyMin = (item: any) => getManualQtyStep(item);
const roundManualQtyByPrecision = (value: any, precision: any) => {
...
const increaseManualQty = (item: any) => {
    const precision = getManualQtyPrecision(item);
    const step = getManualQtyStep(item);
...
const decreaseManualQty = (item: any) => {
    const precision = getManualQtyPrecision(item);
    const step = getManualQtyStep(item);
    const minQty = getManualQtyMin(item);
...
const isProductSelected = (productId: any) =>
    selectedProductIds.value.includes(Number(productId));
const isPendingCustomer = (customerId: any) =>
    String(pendingCustomerId.value) === String(customerId);
const getDefaultUnit = (productId: any) => getUnits(productId)[0] || null;
const editingPriceItem = computed(
    () =>
        cartItems.value.find(
            (item) => String(item.id) === String(editingPriceItemId.value),
        ) || null,
);
...
const openPriceModal = (item: any) => {
    ...
    priceDraftValue.value = formatMoneyInput(
...
const openDiscountModal = () => {
    discountInitialType.value = discountType.value;
    if (discountType.value === "percent") {
        const percent = Math.min(parseAmount(discountValue.value), 100);
        discountInitialValue.value = percent > 0 ? String(percent) : "";
    } else if (discountType.value === "fixed") {
        discountInitialValue.value = formatMoneyInput(discountValue.value);
    } else {
...
const openSurchargeModal = () => {
    surchargeInitialValue.value = formatMoneyInput(surchargeAmount.value);
    showSurchargeModal.value = true;
};

const onSurchargeApply = (value: string) => {
    surchargeAmount.value = value;
    ...
    if (index === null || index < 0 || index >= manualItems.value.length) {
        ...
        currentManualDraft.value = {
            ...
            price_buy: source.price_buy
                ? formatMoneyInput(source.price_buy)
                : "",
            price_sell: source.price_sell
                ? formatMoneyInput(source.price_sell)
                : "",
        };
    }
...
const onManualItemSave = (draft: any) => {
    const parsedBuy = parsePriceShorthand(draft.price_buy);
    const parsedSell = parsePriceShorthand(draft.price_sell);
    const manualQtyPrecision = (() => {
        const raw = String(draft.qty ?? "").trim();
        if (!raw || !raw.includes(".")) return 0;
        const frac = raw.split(".")[1].replace(/[^0-9]/g, "");
        return frac.length;
    })();
    const normalizedManualItem = {
        ...draft,
        qty: formatManualQtyValue(
...
const hasValidManualItems = computed(() =>
    manualItems.value.some(
        (item) => Number(item.qty || 0) > 0 && Number(item.price_sell || 0) > 0,
    ),
);
...
const buildPayload = () => ({
    ...
    customer_id:
        customerMode.value === "existing" && selectedCustomerId.value
...
<template>
    <section class="space-y-4">
        <header>
            <h1 class="text-lg font-medium tracking-tight text-slate-900">
                Bán hàng
            </h1>
        </header>
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="text-sm font-medium text-slate-900">
                                    {{ item.product_name
                                        ...
                                        v-if="getCartItemUnitName(item)"
                                        class="text-slate-500"
                                    >
                                        - {{ getCartItemUnitName(item) }}</span
                                    >
                                </div>
                                <div
            ...
            <section class="app-card">
                ...
                <div class="mt-3 space-y-3">
                    ...
                    <div v-if="showExistingCustomerPicker" class="space-y-1">
                        <span class="mb-1 block text-sm text-slate-700"
                            >Khách cũ</span
                        >
                        <button
                            type="button"
                    ...
                    <div v-if="isPayNow" class="relative">
                        ...
                        />
                        <span
                            class="pointer-events-none absolute inset-y-0 right-3 mt-6 flex items-center text-sm text-slate-500"
                            >đ</span
                        >
                    </div>
        ...
        <Teleport to="body">
            <PriceEditModal
                ...
                :open="showPriceModal"
                :product-label="editingPriceProductLabel"
                :base-price="getCartItemBasePrice(editingPriceItem)"
                :current-price="Number(editingPriceItem?.price || 0)"
                @apply="applyPrice"
            ...
            <transition name="app-modal-fade-up">
                ...
                >
                    <div class="app-modal-sheet app-modal-sheet-fill">
                        ...
                        <div
                            class="app-modal-body app-modal-body-fill pt-2 pb-3"
                        >
                            <div class="mb-2">
                                <input
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        <span
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-brand-50 text-brand-700"
                                        >
                                            <svg
                                                class="h-4 w-4"
                                                viewBox="0 0 20 20"
                                    ...
                                    <span
                                        v-if="isPendingCustomer(customer.id)"
