    ...
    <template v-else-if="supplier">
      ...
      <section v-if="hasOutstandingDebt" class="app-card space-y-4">
        <div>
          ...
          <div class="relative">
            <input v-model="amount" type="text" inputmode="numeric" v-money-input class="h-10 w-full rounded-xl border border-slate-300 px-3 pr-8 text-sm outline-none focus:border-brand-500" />
...
const { supplier, purchases, totalDebt, loading, load } = useSupplierDetail();
...
const submitPayment = async () => {
  if (amountNumber.value <= 0 || !preview.value.length) {
