...
const { formatMoney, formatDateTime, formatHistoryDateTime, formatNumber } =
    useFormat();
...
const noteMeta = computed(() => {
    ...
    if (raw.endsWith("[TT:cash]"))
        return { note: raw.slice(0, -9).trim(), method: "Tiền mặt" };
    if (raw.endsWith("[TT:bank]"))
        return { note: raw.slice(0, -9).trim(), method: "Chuyển khoản" };
...
const pay = async () => {
    try {
        const payload = await submitPayment(Number(route.params.id || 0), {
...
<template>
    <section class="space-y-4">
        <DetailHeaderBar
            :title="
                purchase
                    ? `Phiếu nhập #${purchase.purchase_code}`
                    : `Phiếu nhập #${route.params.id}`
            "
            back-to="/purchases"
        >
        ...
        <div v-else-if="!purchase" class="app-empty-state">
        ...
        <template v-else>
            ...
            >
                ...
                >
                    ...
                    >
                        <span
                            class="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-brand-50 text-brand-700"
                ...
                <div class="space-y-3 px-4 py-4">
                    ...
                    >
                        <span
                            class="inline-flex items-center rounded-lg bg-sky-50 px-2.5 py-1 font-medium text-sky-700"
                            >{{ formatDateTime(purchase.purchase_date) }}</span
                        >
                        <span
                        ...
                        <span
                            v-if="noteMeta.method"
                            class="inline-flex items-center rounded-lg bg-slate-100 px-2.5 py-1 font-medium text-slate-700"
                            >{{ noteMeta.method }}</span
                        >
                    </div>
                    ...
                    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                        ...
                        <div
                            class="rounded-xl border border-brand-100 bg-brand-50 px-4 py-3"
                        >
                            <div
                                class="text-xs uppercase tracking-[0.16em] text-brand-600"
                            >
                                Đã thanh toán
                            </div>
                            <div
                                class="mt-2 text-lg font-semibold text-brand-700"
                            >
                        ...
                        >
                            <div
                                class="text-xs uppercase tracking-[0.16em] text-amber-700"
                            >
                                Còn nợ
                            </div>
                            <div
                    ...
                    <div v-if="noteMeta.note" class="text-sm text-slate-600">
                        <div
                            class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-700 whitespace-pre-line"
                        >
                            {{ noteMeta.note }}
                        </div>
                    </div>
                ...
                <div class="space-y-2 px-4 py-2 text-sm text-slate-700">
                    <div class="flex flex-col gap-x-6 gap-y-1">
                        <div class="flex gap-2">
                            <span class="min-w-24 text-slate-500"
                                >Nhà cung cấp:</span
                            >
                            <RouterLink
                                ...
                                :to="{
                                    ...
                                    params: { id: purchase.supplier_id },
                                }"
                        ...
                        <div v-if="purchase.supplier_phone" class="flex gap-2">
                            ...
                            <div class="inline-flex items-center gap-2">
                                ...
                                <a
                                    :href="`tel:${purchase.supplier_phone}`"
                        ...
                        <div
                            v-if="purchase.supplier_address"
                            class="flex gap-2"
                        >
                            <span class="min-w-24 text-slate-500"
                                >Địa chỉ:</span
            ...
            <PaymentModal
                :open="showPayment && totals.debt > 0"
                title="Thanh toán phiếu nhập"
                :stat-total="totals.total"
                :stat-paid="totals.paid"
                :stat-debt="totals.debt"
                stat-paid-label="Đã trả"
                submit-label="Xác nhận thanh toán"
            ...
            >
                <div
                    class="border-b border-slate-100 px-4 py-3 text-sm font-medium text-slate-800"
                >
                ...
                <div class="divide-y divide-slate-100">
                    ...
                    >
                        <div class="min-w-0">
                            ...
                            <div
                                class="flex flex-wrap gap-x-3 gap-y-1 text-sm mt-1 text-slate-600"
                            >
                                <div>
                                    SL:
                                    <span class="font-medium text-slate-900"
                ...
                <div class="divide-y divide-slate-100">
                    ...
                    >
                        <div class="min-w-0">
                            ...
                            <div
                                v-if="payment.note"
                                class="mt-1 text-slate-600 whitespace-pre-line"
                            >
                                {{ payment.note }}
                            </div>
                        </div>
            ...
            >
                ...
                <p class="text-sm text-slate-500">
                    Nhật ký tạo phiếu, cập nhật mặt hàng và các lần thanh toán
                    của phiếu nhập.
                </p>
                <div
                ...
                >
                    ...
                    >
                        <div class="mb-2">
                            <span
                                class="inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-xs font-medium leading-none text-slate-600"
                                >{{ group.timeText }}</span
                            >
                        </div>
