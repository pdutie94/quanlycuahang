...
import type { Customer, Product, ProductUnit } from '../../../shared/types';
import type { Order, OrderItem, ManualOrderItem, OrderFormState } from '../types';
...
function createEmptyManualItem(): ManualOrderItem {
  return { item_name: '', unit_name: '', qty: '1', price_buy: '', price_sell: '' };
}
...
export function useOrderForm() {
  ...
  const form = ref<OrderFormState>(createDefaultFormState());
  const rows = ref<OrderItem[]>([]);
  const manualItems = ref<ManualOrderItem[]>([]);
  const existingItemIds = ref<number[]>([]);
  ...
  const summary = computed(() => {
    let subtotal = 0;
    for (const row of rows.value) subtotal += Number(row.qty || 0) * parseMoneyValue(row.price || 0);
    let manualSellTotal = 0;
    for (const row of manualItems.value) manualSellTotal += Number(row.qty || 0) * parseMoneyValue(row.price_sell || 0);
    return { subtotal, manualSellTotal, gross: subtotal + manualSellTotal };
  });

  const loadBootstrap = async () => {
    const payload = await bootstrapRequest.execute();
    const data = payload?.data || {};
    products.value = (data.products as any[]) || [];
    productUnitsByProduct.value = data.product_units_by_product || {};
    customers.value = data.customers || [];
    productUnits.value = products.value.flatMap((product: Product) => {
      const units = productUnitsByProduct.value[String(product.id)] || [];
      return units.map((unit: any) => ({
        ...
        allow_fraction: Number(unit.allow_fraction || 0),
        min_step: Number(unit.min_step || 1)
      }) as ProductUnit);
    });
    return payload;
  };
  ...
  const loadEdit = async (id: number | string) => {
    ...
    if (!orderData) return payload;
    const noteMeta = parseMethodAndNote(orderData.note || '');
    const rawDiscountType = String(orderData.discount_type || 'none');
    let normalizedDiscountType: 'none' | 'fixed' | 'percent' = ['none', 'fixed', 'percent'].includes(rawDiscountType)
      ? (rawDiscountType as any)
      : 'none';
    const normalizedDiscountAmount = Number(orderData.discount_amount || 0);
    if (normalizedDiscountType === 'none' && normalizedDiscountAmount > 0) normalizedDiscountType = 'fixed';
    const normalizedDiscountValue = normalizedDiscountType === 'fixed'
      ? String(normalizedDiscountAmount || orderData.discount_value || '')
      : (normalizedDiscountType === 'percent' ? String(orderData.discount_value || '') : '');
    form.value = {
      customer_id: orderData.customer_id ? String(orderData.customer_id) : '',
      customer_name: orderData.customer_name || '',
      customer_phone: orderData.customer_phone || '',
      customer_address: orderData.customer_address || '',
      order_date: toDateTimeLocalValue(orderData.order_date),
      note: noteMeta.note,
    ...
    existingItemIds.value = (data.items || []).map((item: any) => Number(item.id || 0)).filter((idValue: number) => idValue > 0);
    manualItems.value = (data.manual_items || []).map((item: any) => ({
  ...
  const addRow = (initial: Partial<OrderItem> | null = null) => {
    rows.value.push({ source_id: 0, product_unit_id: initial?.product_unit_id || '', qty: initial?.qty || '1', price: initial?.price || '' });
  };
  const removeRow = (index: number) => rows.value.splice(index, 1);
  const addManualItem = (initial: Partial<ManualOrderItem> | null = null) => manualItems.value.push({ ...createEmptyManualItem(), ...(initial || {}) });
  const removeManualItem = (index: number) => manualItems.value.splice(index, 1);

  const syncCustomerFields = () => {
    const selected = customers.value.find((customer: Customer) => Number(customer.id) === Number(form.value.customer_id));
    if (!selected) return;
  ...
  const resetState = () => {
    ...
    rows.value = [];
    manualItems.value = [];
    existingItemIds.value = [];
  };

  const buildCreatePayload = () => ({
    items: rows.value.map((row: OrderItem) => {
      const unit = rowDisplayMap.value.get(String(row.product_unit_id));
      return { product_id: Number(unit?.product_id || 0), unit_id: Number(unit?.unit_id || 0), quantity: Number(row.qty || 0), price: parseMoneyValue(row.price || 0) };
    }),
    ...
    manual_item_name: manualItems.value.map((item: ManualOrderItem) => item.item_name),
    manual_unit_name: manualItems.value.map((item: ManualOrderItem) => item.unit_name),
    manual_qty: manualItems.value.map((item: ManualOrderItem) => item.qty),
    manual_price_buy: manualItems.value.map((item: ManualOrderItem) => item.price_buy),
    manual_price_sell: manualItems.value.map((item: ManualOrderItem) => item.price_sell)
  });
  ...
  const buildUpdatePayload = () => ({
    ...
    product_unit_id: rows.value.map((row: OrderItem) => row.product_unit_id),
    qty: rows.value.map((row: OrderItem) => row.qty),
    price: rows.value.map((row: OrderItem) => parseMoneyValue(row.price)),
    mode: rows.value.map(() => 'new'),
    manual_item_name: manualItems.value.map((item: ManualOrderItem) => item.item_name),
    manual_unit_name: manualItems.value.map((item: ManualOrderItem) => item.unit_name),
    manual_qty: manualItems.value.map((item: ManualOrderItem) => item.qty),
    manual_price_buy: manualItems.value.map((item: ManualOrderItem) => parseMoneyValue(item.price_buy)),
    manual_price_sell: manualItems.value.map((item: ManualOrderItem) => parseMoneyValue(item.price_sell))
  });

  const submitCreate = async () => createRequest.execute(buildCreatePayload());
  const submitUpdate = async (id: number | string) => updateRequest.execute(id, buildUpdatePayload());
