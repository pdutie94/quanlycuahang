...
export function useOrderForm() {
  ...
  const summary = computed(() => {
    ...
    for (const row of rows.value) {
      subtotal += Number(row.qty || 0) * parseMoneyValue(row.price || 0);
    }

    let manualSellTotal = 0;
    for (const row of manualItems.value) {
      manualSellTotal += Number(row.qty || 0) * parseMoneyValue(row.price_sell || 0);
    }
  ...
  const loadEdit = async (id) => {
    ...
    let normalizedDiscountType = ['none', 'fixed', 'percent'].includes(rawDiscountType) ? rawDiscountType : 'none';
    const normalizedDiscountAmount = Number(orderData.discount_amount || 0);

    if (normalizedDiscountType === 'none' && normalizedDiscountAmount > 0) {
  ...
  const buildCreatePayload = () => ({
    items: rows.value.map((row) => {
      ...
      return {
        ...
        price: parseMoneyValue(row.price || 0)
      };
    }),
  ...
  const buildUpdatePayload = () => ({
    ...
    price: rows.value.map((row) => parseMoneyValue(row.price)),
    mode: rows.value.map(() => 'new'),
    manual_item_name: manualItems.value.map((item) => item.item_name),
    manual_unit_name: manualItems.value.map((item) => item.unit_name),
    manual_qty: manualItems.value.map((item) => item.qty),
    manual_price_buy: manualItems.value.map((item) => parseMoneyValue(item.price_buy)),
    manual_price_sell: manualItems.value.map((item) => parseMoneyValue(item.price_sell))
  });
