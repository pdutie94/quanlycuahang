...
const onPaymentAmountInput = (event: Event) => {
    ...
    const rawValue =
        target instanceof HTMLInputElement ? target.value : paymentAmount.value;
...
const discountAmount = computed(() =>
    Math.max(Number(order.value?.discount_amount || 0), 0),
);
...
const isPaid = computed(
    () => order.value?.status === "paid" || remainingAmount.value <= 0,
);
const orderStatus = computed(() => order.value?.order_status || "pending");
const canCollectPayment = computed(
    () => remainingAmount.value > 0 && orderStatus.value !== "cancelled",
);
const canResetPayment = computed(
    () => order.value?.status === "paid" && orderStatus.value !== "cancelled",
);
...
const submitStatusForm = async () => {
    ...
    try {
        ...
        toast.error(
            statusError.value || "Không thể cập nhật trạng thái đơn hàng.",
        );
    }
};
...
const parseLogLine = (detailRaw: any) => {
    ...
    try {
        ...
        if (detail.type === "payment") {
            const methodText = detail.method_text
                ? ` (${detail.method_text})`
                : "";
            const remainingAfter =
                detail.remaining_after !== undefined &&
                detail.remaining_after !== null
...
<template>
    <section class="space-y-4">
        <DetailHeaderBar
            :title="
                order ? `Đơn hàng #${order.order_code}` : `Đơn hàng #${orderId}`
            "
            back-to="/orders"
        >
            <template #actions="{ closeMenu }">
                <template v-if="order">
                    <RouterLink
                        ...
                        class="detail-header-menu-item"
                        @click="closeMenu"
                        ><FileText class="h-4 w-4 shrink-0" /><span
                            >In hóa đơn</span
                        ></RouterLink
                    >
                    <button
                        ...
                        "
                    >
                        <BanknoteArrowDown class="h-4 w-4 shrink-0" /><span
                            >Thu tiền</span
                        >
                    </button>
                    <button
                        v-if="canResetPayment"
                        type="button"
                        class="detail-header-menu-item detail-header-menu-item-amber"
                        @click="
                            closeMenu();
                            showResetPaymentModal = true;
                        "
                    >
                    ...
                    <RouterLink
                        ...
                        class="detail-header-menu-item"
                        @click="closeMenu"
                        ><Pencil class="h-4 w-4 shrink-0" /><span
                            >Sửa đơn</span
                        ></RouterLink
                    >
                    <RouterLink
                        ...
                        @click="closeMenu"
                        ><Undo2 class="h-4 w-4 shrink-0" /><span
                            >Trả hàng</span
                        ></RouterLink
                    >
                    <button
                        ...
                        "
                    >
                        <Trash2 class="h-4 w-4 shrink-0" /><span
                            >Xóa đơn hàng</span
                        >
                    </button>
                </template>
        ...
        <template v-else>
            <div class="space-y-4">
                <section class="rounded-lg border border-slate-200 bg-white">
                    <div
                        class="flex items-center justify-between border-b border-slate-100 px-4 py-2"
                    >
                        <div
                        ...
                        >
                            ...
                            /></span>
                            <span
                                class="text-sm font-medium text-slate-900 sm:text-sm"
                    ...
                    <div class="px-4 py-2">
                        <div
                            class="flex flex-wrap items-center gap-2 text-sm text-slate-600 sm:text-sm"
                        >
                            <span
                                class="inline-flex items-center rounded-lg bg-sky-50 px-2.5 py-0.5 text-sm font-medium text-sky-700"
                                >{{ formatDateTime(order.order_date) }}</span
                            >
                            <span
                        ...
                        <div class="mt-3 space-y-2 text-sm">
                            <div class="flex items-center justify-between">
                                <span
                                    class="font-medium uppercase text-slate-500"
                                    >Tổng vốn</span
                                >
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                ...
                                <div class="rounded-md bg-amber-50 px-3 py-2">
                                    <div
                                        class="text-sm font-medium uppercase text-amber-700"
                                    >
                                        Còn nợ
                                    </div>
                                    <div
                        ...
                        <div
                            class="mt-3 border-t border-slate-100 pt-3 text-sm text-slate-700"
                        >
                            <form
                                class="flex flex-wrap items-center gap-2"
                                @submit.prevent="submitStatusForm"
                            >
                                ...
                                <div class="relative">
                                    <select
                                        ...
                                        class="form-field block w-full appearance-none rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 pr-8 text-sm outline-none transition focus:border-brand-500"
                                    >
                    ...
                    <div class="space-y-2 px-4 py-2 text-sm text-slate-700">
                        <div class="flex flex-col gap-x-6 gap-y-1">
                            <div class="flex gap-2">
                                <span class="min-w-20 text-slate-500"
                                    >Khách hàng:</span
                                >
                            ...
                            <div
                                v-if="noteMeta.paymentMethodText"
                                class="flex gap-2"
                            >
                                <span class="min-w-20 text-slate-500"
                                    >Thanh toán:</span
                                >
                                <span class="font-medium text-slate-900">{{
                                    noteMeta.paymentMethodText
                                }}</span>
                            </div>
                            <div
                                v-if="noteMeta.note"
                                class="rounded-md bg-brand-50 px-3 py-2 text-slate-700 whitespace-pre-line"
                            >
                                {{ noteMeta.note }}
                            </div>
                        </div>
                ...
                <section
                    v-if="!items.length && !manualItems.length"
                ...
                >
                    ...
                    >
                        ...
                        >
                            <span
                                class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-slate-50 text-slate-700"
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                >
                                    <div class="min-w-0">
                                        ...
                                        <div
                                            class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-600"
                                        >
                                            <div
                ...
                >
                    <div
                        class="border-b border-slate-100 px-4 py-2 text-sm font-medium text-slate-800"
                    >
                        Lịch sử thanh toán
                    </div>
                    <div class="divide-y divide-slate-100">
                        <div
                            ...
                            class="flex flex-col gap-x-2 gap-y-1 px-4 py-3 text-sm sm:flex-row sm:items-start sm:justify-between"
                        >
                            <div class="min-w-0">
                            ...
                            <div
                                class="flex flex-col gap-1 text-sm text-slate-500"
                            >
                ...
                <section
                    v-if="groupedLogs.length"
                    class="rounded-2xl border border-slate-200 bg-white p-4 space-y-3"
                >
                    <h2
                        class="flex items-center gap-2 text-base font-medium text-slate-800"
                    >
                        <span
                            class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-violet-100 text-violet-700"
                        >
                    ...
                    <p class="text-sm text-slate-500">
                        Nhật ký các lần thay đổi trạng thái, thanh toán và cập
                        nhật mặt hàng của đơn.
                    </p>
                    <div
                        class="max-h-72 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50/70"
                    >
                        <div
                        ...
                        >
                            <div class="mb-2">
                                <span
                                    class="leading-none inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600"
                                    >{{ group.timeText }}</span
                                >
                            </div>
                            <ul class="space-y-1 text-sm">
                                ...
                                >
                                    <div class="pt-1">
                                        <span
                                            class="mt-[2px] block h-2 w-2 rounded-full"
                                            :class="entry.tone.dot"
                                        ></span>
                                    </div>
                                    <div
                                        class="min-w-0 flex-1 leading-5"
                                        :class="entry.tone.detail"
                                    >
                                        {{ entry.parsed.text }}
                                    </div>
                                </li>
            ...
            <PaymentModal
                :open="showPaymentModal && canCollectPayment"
                title="Thu tiền đơn hàng"
                :stat-total="Number(order?.total_amount || 0)"
                :stat-paid="Number(order?.paid_amount || 0)"
                :stat-debt="remainingAmount"
                stat-paid-label="Đã thu"
                submit-label="Xác nhận thu"
