...
const { formatMoney, formatCompactMoney, formatNumber } = useFormat();
...
<template>
  <section class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-4"><div class="flex items-center gap-3"><DollarSign class="h-5 w-5 text-amber-600" /><div><p class="text-sm text-slate-500">Tổng doanh thu</p><p class="text-lg font-semibold" :title="formatMoney(summary.total_revenue)">{{ formatCompactMoney(summary.total_revenue) }}</p></div></div></div>
    ...
    <template v-else>
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-3"><div class="mb-3 flex flex-wrap items-center justify-between gap-3"><h2 class="text-base font-medium">Tất cả sản phẩm</h2><p class="text-sm text-slate-500">{{ meta.total_count.toLocaleString() }} sản phẩm · trang {{ meta.page }}/{{ meta.total_pages }}</p></div><p v-if="!items.length" class="py-6 text-center text-sm text-slate-500">Không có sản phẩm để hiển thị.</p><div v-else class="overflow-x-auto"><table class="min-w-[44rem] w-full text-sm"><thead><tr class="border-b text-left text-slate-600"><th class="pb-2 pr-4">Sản phẩm</th><th class="px-2 pb-2 text-right">Tồn kho</th><th class="px-2 pb-2 text-right">Đã bán</th><th class="px-2 pb-2 text-right">Doanh thu</th><th class="px-2 pb-2 text-right">Lợi nhuận</th><th class="pl-2 pb-2 text-right">Tỷ suất LN</th></tr></thead><tbody><tr v-for="product in items" :key="product.id" class="border-b border-slate-100" :class="{ 'bg-slate-50': product.sold_qty === 0 }"><td class="py-2 pr-4"><div class="flex items-baseline gap-2 whitespace-nowrap"><span class="font-medium">{{ product.name }}</span><span class="text-xs text-slate-500">{{ product.code }}</span></div></td><td class="px-2 py-2 text-right whitespace-nowrap">{{ formatNumber(product.current_stock) }}</td><td class="px-2 py-2 text-right whitespace-nowrap">{{ formatNumber(product.sold_qty) }}</td><td class="px-2 py-2 text-right whitespace-nowrap" :title="formatMoney(product.revenue)">{{ formatCompactMoney(product.revenue) }}</td><td class="px-2 py-2 text-right whitespace-nowrap" :class="product.profit > 0 ? 'text-green-600' : 'text-slate-400'" :title="formatMoney(product.profit)">{{ formatCompactMoney(product.profit) }}</td><td class="pl-2 py-2 text-right whitespace-nowrap">{{ product.profit_margin }}%</td></tr></tbody></table></div><div v-if="meta.total_pages > 1" class="mt-3 flex items-center justify-end gap-3"><button type="button" class="rounded-lg border px-3 py-1.5 text-sm disabled:opacity-50" :disabled="meta.page <= 1" @click="changePage(meta.page - 1)">Trước</button><button type="button" class="rounded-lg border px-3 py-1.5 text-sm disabled:opacity-50" :disabled="meta.page >= meta.total_pages" @click="changePage(meta.page + 1)">Sau</button></div></div>
    </template>
  </section>
</template>
