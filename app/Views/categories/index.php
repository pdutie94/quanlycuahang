<div class="mb-4 space-y-3">
    <div class="flex items-center justify-between gap-3">
        ...
        <button type="button" data-category-toggle class="inline-flex items-center justify-center rounded-full bg-emerald-600 p-2 text-white shadow-sm hover:bg-emerald-700 active:bg-emerald-800">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5">
					<path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
				</svg>
        </button>
    </div>
    <div class="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-sm hidden" data-category-form>
...
<?php } else { ?>
	<div class="space-y-3" data-infinite-list data-infinite-url="<?php echo $basePath; ?>/category" data-current-page="<?php echo isset($page) ? (int) $page : 1; ?>" data-has-more="<?php echo isset($totalPages) && isset($page) && $page < $totalPages ? '1' : '0'; ?>">
		<?php foreach ($categories as $category) { ?>
			<div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm" data-infinite-item>
