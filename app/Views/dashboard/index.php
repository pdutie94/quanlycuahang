...
<div class="space-y-4">
	...
	<div class="flex items-center justify-between gap-3">
		...
		<span class="inline-flex rounded-chip border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700"><?php echo date('d/m/Y'); ?></span>
	</div>
	...
	<div class="space-y-2">
		...
		<div class="grid grid-cols-2 gap-2 xl:grid-cols-4">
			<a href="<?php echo $basePath; ?>/pos" class="app-shortcut-link">
				<span class="app-shortcut-icon">
				<?php echo ui_icon('cart', 'h-4 w-4'); ?>
				</span>
				<span>Tạo đơn</span>
			</a>
			<a href="<?php echo $basePath; ?>/order" class="app-shortcut-link">
				<span class="app-shortcut-icon">
				<?php echo ui_icon('clipboard-document', 'h-4 w-4'); ?>
				</span>
				<span>Đơn hàng</span>
			</a>
			<a href="<?php echo $basePath; ?>/product/create" class="app-shortcut-link">
				<span class="app-shortcut-icon">
				<?php echo ui_icon('plus', 'h-4 w-4'); ?>
				</span>
				<span>Thêm SP</span>
			</a>
			<a href="<?php echo $basePath; ?>/report" class="app-shortcut-link">
				<span class="app-shortcut-icon">
		...
		<div class="space-y-2">
			...
			<div class="space-y-2">
				<?php foreach ($lowStockPreview as $row) { ?>
					...
					?>
					<div class="flex items-center justify-between rounded-card border border-rose-100 bg-white px-4 py-3 text-sm">
						<div class="min-w-0">
						...
						<?php if ($ratio <= 0.3) { ?>
							<span class="ml-2 rounded-lg bg-rose-600 px-2 py-0.5 text-sm font-medium text-white">Rất thấp</span>
						<?php } elseif ($ratio <= 0.7) { ?>
							<span class="ml-2 rounded-lg bg-rose-500 px-2 py-0.5 text-sm font-medium text-white">Sắp hết</span>
						<?php } else { ?>
							<span class="ml-2 rounded-lg bg-rose-400 px-2 py-0.5 text-sm font-medium text-white">Thấp</span>
						<?php } ?>
					</div>
				<?php } ?>
			</div>
