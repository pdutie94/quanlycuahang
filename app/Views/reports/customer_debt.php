...
<div class="space-y-4">
	<div class="grid grid-cols-1 gap-3 md:grid-cols-3">
		...
		<div class="rounded-xl border border-brand-100 bg-brand-50 px-3 py-3 text-sm text-brand-900 ">
			<div class="text-sm font-medium uppercase  text-brand-700">Đã thu</div>
					...
					<tbody class="divide-y divide-slate-100 bg-white">
						<?php foreach ($rows as $row) { ?>
							<tr class="hover:bg-slate-50">
								...
								<td class="px-3 py-2 align-top text-right">
									<?php if (!empty($row['id'])) { ?>
										<a href="<?php echo $basePath; ?>/customer/view?id=<?php echo (int) $row['id']; ?>" class="inline-flex items-center rounded-lg border border-slate-300 px-2 py-1 text-sm font-medium text-slate-700 hover:bg-slate-100">
											Xem đơn
										</a>
									<?php } ?>
								</td>
