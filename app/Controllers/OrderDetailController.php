...
class OrderDetailController extends Controller
{
    ...
    public function preview()
    {
        ...
        $stmt = $pdo->prepare('SELECT oi.*, p.name AS product_name, u.name AS unit_name FROM order_items oi JOIN products p ON oi.product_id = p.id JOIN product_units pu ON oi.product_unit_id = pu.id JOIN units u ON pu.unit_id = u.id WHERE oi.order_id = ? ORDER BY oi.id');
        $stmt->execute([$id]);
        $items = $stmt->fetchAll();
    ...
    public function update()
    {
        ...
        try {
            ...
            $orderDate = OrderService::normalizeOrderDate($orderDateInput, $orderDate);
